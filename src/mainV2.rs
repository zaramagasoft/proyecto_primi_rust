...
pub struct ModelConfig {
    ...
    #[config(default = 512)] hidden_size: usize, // --- POTENCIA DOBLADA ---
...
fn entrenar_y_predecir(sorteos_raw: Vec<Sorteo>, reintegros_raw: Vec<Reintegro>) {
    ...
    let inputs_tensor: Tensor<MyAutodiffBackend, 2> = Tensor::from_floats(TensorData::new(datos, [num_sorteos, 6]), &device);
    ...
    println!("--> Entrenando Números con 512 neuronas (5000 épocas)...");
    for epoch in 1..=5000 { // --- MÁS PROFUNDIDAD ---
        ...
        model = optim.step(0.0005, model, grads); // LR más fino
        if epoch % 1000 == 0 { println!("Época [{}] - Refinando patrones...", epoch); }
    }
    ...
    let tensor_r: Tensor<MyAutodiffBackend, 2> = Tensor::from_floats(TensorData::new(datos_r, [num_r, 1]), &device);
    let mut model_r = LinearConfig::new(1, 32).init::<MyAutodiffBackend>(&device); // Subimos a 32 neuronas
    ...
    println!("\n-------------------------------------------");
    println!("   BOLETO ACTUALIZADO R9 390 (512 NEURONAS) ");
    println!("-------------------------------------------");
